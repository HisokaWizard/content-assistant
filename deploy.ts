...
const envSchema = z.object({
  ...
  OPENCODE_MODEL: z.string().default('opencode/minimax-m2.5-free'),
  ...
  YTDLP_PATH: z.string().default('/opt/homebrew/bin/yt-dlp'),
});
...
Env (bot/.env):
  ...
  OPENCODE_MODEL    Model (default: opencode/minimax-m2.5-free)
...
async function main() {
  ...
  await execa('pm2', ['delete', env.PM2_OPENCODE_NAME], { cwd: rootDir }).catch(() => {});
  await run('pm2', ['start', 'opencode', '--name', env.PM2_OPENCODE_NAME, '--', ...opencodeArgs], {
    cwd: rootDir,
    env: opencodeEnv,
  });
  ...
  await execa('pm2', ['delete', env.PM2_BOT_NAME], { cwd: rootDir }).catch(() => {});
  await run('pm2', ['start', 'npm', '--name', env.PM2_BOT_NAME, '--', 'start'], {
    cwd: botDir,
    env: {
      OPENCODE_URL: opencodeUrl,
